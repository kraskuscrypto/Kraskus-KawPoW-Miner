...
- Boost.Asio port: pool clients (stratum, getwork), API server, farm and CLI now use `io_context`, `strand<io_context::executor_type>`, `bind_executor`, `boost::asio::post`, the resolver `results_type` API, `steady_timer`/`std::chrono` instead of `deadline_timer`/`posix_time`, `make_address`, `streambuf::data().data()` and `ssl::host_name_verification` (everything Boost >= 1.87 removed). The build defines `BOOST_ASIO_NO_DEPRECATED` so older Boost versions reject deprecated calls too. The `reboot.sh`/`reboot.bat` hook keeps Boost.Process v1 (`<boost/process/v1.hpp>`, deprecated upstream but still shipped in 1.92); whether the fork should keep that hook at all under the Universal Miner is an open product question. Windows: vcpkg manifest (`vcpkg.json`) pinned to a current baseline (Boost 1.92.0 / OpenSSL 3.6.4), static x64.
- Windows x64: builds in CI against vcpkg Boost 1.92 / OpenSSL 3.6.4 (static CRT); CUDA host code compiled with MSVC 14.44 + CUDA 12.8.
- Not yet done (tracked in the Universal Miner Phase 3 checkpoint): real-pool shares on the RTX 3070 Ti (needs the owner's RVN address), a Windows GPU run, GTX 1070 / RTX 5070 gates.
