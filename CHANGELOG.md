...
- Boost.Asio port: pool clients (stratum, getwork), API server, farm and CLI now use `io_context`, `strand<io_context::executor_type>`, `bind_executor`, `boost::asio::post`, the resolver `results_type` API, `steady_timer`/`std::chrono` instead of `deadline_timer`/`posix_time`, `make_address`, `streambuf::data().data()` and `ssl::host_name_verification` (everything Boost >= 1.87 removed). The build defines `BOOST_ASIO_NO_DEPRECATED` so older Boost versions reject deprecated calls too. Windows: vcpkg manifest (`vcpkg.json`) pinned to a current baseline (Boost 1.92.0 / OpenSSL 3.6.4), static x64.
